...
def split_data(clients_num):
    ...
    for i in range(len(y_train)):
    	y_train[i] = tf.squeeze(y_train[i], axis=1)
...
# 划分客户端数据集
CLIENT_NUM = 20
X_train, y_train, X_test, y_test = split_data(CLIENT_NUM)
...
def run_one_node(node_id):
    ...
    def local_training():
        ...
        try:
            ...
            variables = model_v
            for i in range(len(conv_net.trainable_variables)):
                print(conv_net.trainable_variables[i].shape)
            # 转为tf数据格式
            for i in range(len(variables)):
                variables[i] = tf.cast(variables[i], dtype=tf.float32)
            # print("node_{} updating!!!!".format(node_id))
            ...
            print("node_{} had updated".format(node_id))
